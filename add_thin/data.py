...
class Batch:
    ...
    def thin(self, alpha: TensorType[float]) -> Tuple["Batch", "Batch"]:
        ...
        if alpha.dim() == 1:
            keep = torch.bernoulli(alpha.unsqueeze(1)).bool()
        elif alpha.dim() == 2:
            keep = torch.bernoulli(alpha).bool().to(device)
        elif alpha.dim() == 0:
            alpha_1 = alpha.unsqueeze(0)
            keep = torch.bernoulli(alpha_1.unsqueeze(1)).bool()
        else:
            raise Warning("alpha has too many dimensions")

        # remove from mask
        keep_mask = self.mask.to(device) * keep
        rem_mask = self.mask.to(device) * ~keep
...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def bounded_subset(self, lower: int, upper: int) -> "SequenceDataset":
        """Return a new SequenceDataset with elements in [lower, upper)."""
        if lower < 0 or upper > len(self.sequences):
            raise IndexError("Subset bounds are out of dataset range.")
        return SequenceDataset(self.sequences[lower:upper])
...
@typechecked
class Student_DataModule(pl.LightningDataModule):
    """
    Datamodule for variable length event sequences for temporal point processes.
    ...
    def __init__(
        ...
        self.test_data = None
        self.train_data = None
        self.x0train_data = None
        self.x0val_data = None
        self.x0test_data = None
        self.xNtrain_data= None
        self.xNval_data= None
        self.xNtest_data = None


    def prepare_data(self) -> None:
        x_0time_sequences , x_Ntime_sequences, tmax = load_student_sequences(self.root, self.name, self.tmax_name)
        ...
        self.x0dataset = SequenceDataset(sequences=x_0time_sequences)
        self.tmax = tmax
        self.x0train_size = int(self.train_percentage * len(self.x0dataset))
        self.x0val_size = int(self.val_percentage * len(self.x0dataset))
        self.x0test_size = len(self.x0dataset) - (self.x0train_size + self.x0val_size)

        self.x0train_data, self.x0val_data, self.x0test_data = random_split(
            self.x0dataset,
            [self.x0train_size, self.x0val_size, self.x0test_size],
            generator=torch.Generator().manual_seed(self.split_seed),
        )

        self.xNdataset = SequenceDataset(sequences=x_Ntime_sequences)
        self.xNtrain_size = int(self.train_percentage * len(self.xNdataset))
        self.xNval_size = int(self.val_percentage * len(self.xNdataset))
        self.xNtest_size = len(self.xNdataset) - (self.xNtrain_size + self.xNval_size)

        self.xNtrain_data, self.xNval_data, self.xNtest_data = random_split(
            self.xNdataset,
            [self.xNtrain_size, self.xNval_size, self.xNtest_size],
            generator=torch.Generator().manual_seed(self.split_seed),
        )
    ...
    def get_statistics(self):
        # Get train stats
        seq_lengths = []
        for i in range(len(self.x0train_data)):
            seq_lengths.append(len(self.x0train_data[i]))
        self.n_max = max(seq_lengths)
    ...
    def val_dataloader(self) -> DataLoader:
        return DataLoader(
            self.x0val_data,
            batch_size=len(self.x0val_data),  # evaluate all at once
            collate_fn=Batch.from_sequence_list,
            num_workers=0,
            drop_last=False,
        )
...
def load_sequences(root, name: str) -> List[Sequence]:
    ...
    # for sequence_time in time:
    #     print(f"time sequence {sequence_time.shape}: {sequence_time}, tmax: {tmax}, ")
    return time_sequences


def pad_list_of_lists(data, pad_value=0):
    max_rows = max(len(arr) for arr in data)
    max_cols = max(max(len(row) for row in arr) for arr in data)

    padded = []
    for arr in data:
        padded_arr = []
        for row in arr:
            row_pad = np.pad(row, (pad_value, max_cols - len(row)), mode='constant')  # shape (2201,)
            padded_arr.append(row_pad)
        for _ in range(max_rows - len(arr)):  # pad rows
            padded_arr.append([pad_value] * max_cols)
...
def load_student_sequences(root, name: str, tmax_name: str) -> List[Sequence]:
    """Load dataset.
    ...
    tmax_path = os.path.join(root, f"{tmax_name}.npy")
    # Load the list of pair
    # [[x_0, x_N]]
    load_pair = np.load(data_path, allow_pickle=True)
    load_tmax = np.load(tmax_path)
    # List of array list[array]
    x_0 = []
    # List of tensor list[tensor]
    x_N = []
    x_N_Kept = []
    for pair in load_pair:
        x_0.append(pair[0])
        x_N.append(pair[1]['time'].cpu().numpy())
        x_N_Kept.append(pair[1]['kept'].cpu().numpy())


    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    tmax = float(load_tmax[0])

    count = 0
    x_0time_sequences = []
    x_Ntime_sequences = []
    for index in range(len(x_0)):
        for i, t in enumerate(x_0[index]):
            count = count + 1
            # time_sequences.append(Sequence(np.concatenate([x_0[0][i], x_N[0][i]], axis=0), tmax=tmax))
            x_0time_sequences.append(Sequence(x_0[index][i], tmax=tmax))
            x_Ntime_sequences.append(Sequence(x_N[index][i], kept_points=x_N_Kept[0][i], tmax=tmax))
            # if count >= 10:
            #     break
    print(f"Total number of pairs are {count}.")
    return x_0time_sequences, x_Ntime_sequences, tmax